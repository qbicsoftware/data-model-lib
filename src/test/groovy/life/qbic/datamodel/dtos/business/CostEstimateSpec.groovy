...
class CostEstimateSpec extends Specification {
    ...
    def "Fluent API shall create a CustomerEstimate object"() {
        ...
        CostEstimate costEstimate =
                new CostEstimate.Builder(date).expirationDate(date).customer(customer).projectManager(projectManager)
                        .projectDescription("Cartoon Series").projectTitle("Archer").items([]).totalPrice(price)
                        .identifier(costEstimateId).selectedCustomerAffiliation(selectedAffiliation)
                        .build()
