...
class ExperimentalDesignTemplate extends DataFile{
    ...
    /**
     * Creates the ExperimentalDesignTemplate object based on a file name and a relative path of it
     * @param name The file name of the ExperimentalDesignTemplate file
     * @param relativePath The relative path to the file in a file system
     * @return the ExperimentalDesignTemplate object, which also validates if the filename is valid
     * @since 2.10.0
     */
    ...
    private void validateName() {
        if (!(this.name =~ NAME_SCHEMA)) {
            throw new IllegalArgumentException("Name must match the MaxQuant ExperimentalDesignTemplate schema!")
        }
    }
}
