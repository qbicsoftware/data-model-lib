...
def main():
    parser = argparse.ArgumentParser(description='QBiC Javadoc Generator.',
                                     prog='generate-javadocs.py',
    ...
    parser.add_argument('-a', '--access-token-var-name',
                        default=TOKEN_ENV_VARIABLE_NAME,
                        help='Name of the environment variable holding the GitHub personal access token used to push changes in reports.')
    parser.add_argument('-r', '--validation-regex',
                        default=REPORTS_VERSION_REGEX,
...
def validateArguments(args):
    ...
    if not args.access_token_var_name in os.environ:
        raise Exception(
            'At least one of the required environment variables is missing. See comments on .generate-reports.py for further information.')
    ...
    if not regex.match(args.output_dir):
        raise Exception(
            'The provided output directory for the reports, {}, is not valid. It must match the regex {}'.format(
                args.output_dir, args.validation_regex))
    ...
    if not os.path.exists(args.site_dir) or not os.path.isdir(args.site_dir):
        raise Exception(
            'Maven site folder {} does not exist or is not a directory.'.format(
                args.site_dir))
...
def clone_self(working_dir, args, exit_if_fail=True):
    execute(['git', 'clone', 'https://{}:x-oauth-basic@github.com/{}'.format(
        os.environ[args.access_token_var_name], args.repo_slug), working_dir],
            'Could not clone {} in directory {}'.format(args.repo_slug,
                                                        working_dir),
            exit_if_fail)
...
def force_checkout_pages_branch(working_dir, args):
    ...
    try:
        ...
        execute(['git', '-C', working_dir, 'checkout', '-b', args.pages_branch],
                'Could not create branch {}'.format(args.pages_branch))
...
def remove_unneeded_files(working_dir, args):
    print('Cleaning local repository ({}) of non-reports files'.format(
        working_dir))
    for f in os.listdir(working_dir):
        if should_delete(f, args):
            ...
            execute(
                ['git', '-C', working_dir, 'rm', '-r', '--ignore-unmatch', f],
                'Could not remove {}.'.format(f))
...
def prepare_report_dir(working_dir, args):
    report_output_dir = os.path.join(working_dir, args.base_output_dir,
                                     args.output_dir)
    if os.path.exists(report_output_dir):
        if not os.path.isdir(report_output_dir):
            print(
                'WARNING: Output destination {} exists and is not a directory.'.format(
                    report_output_dir), file=sys.stderr)
        # remove the object from git
        print('Removing {}'.format(report_output_dir))
        execute(['git', '-C', working_dir, 'rm', '-r', '--ignore-unmatch',
                 os.path.join(args.base_output_dir, args.output_dir)],
    ...
    print(
        'Moving contents of {} to {}'.format(args.site_dir, report_output_dir))
...
def push_to_pages_branch(working_dir, args):
    if args.dry_run:
        print(
            '(running in dry run mode) Local/remote repository will not be modified')
    else:
        # add changes to the index
        print('Staging changes for commit')
        execute(['git', '-C', working_dir, 'add', '.'],
                'Could not stage reports for commit.')
        ...
        execute(['git', '-C', working_dir, 'push', '-u', 'origin',
                 args.pages_branch],
                'Could not push changes using provided credentials.')
...
def should_delete(path, args):
    return not UNTOUCHABLE_FILES_MATCHER.match(
        path) and path != args.base_output_dir
...
def execute(command, error_message='Error encountered while executing command',
    ...
    try:
        ...
        if exit_if_fail:
            ...
            try:
                print('{}\n  Error originated at file {}, line {}'.format(
                    error_message, stack[-2].filename, stack[-2].lineno),
                      file=sys.stderr)
            except:
                print(
                    '{}\n  No information about the originating call is available.'.format(
